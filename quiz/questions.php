...
<html>
	<head>
		<title>Exousia 14 Quiz Event</title>
...
$category='';
 if(!empty($_POST['uid'])){
    ...
    $uid=str_replace("EX14","",$uid);
	 ...
	 if($valid>0)
		 {
	 $res= mysqli_query( $con,"select * from quiz_attendee where uid='$uid'") or die(mysql_error()); 
    ...
    mysqli_query( $con, "INSERT INTO quiz_attendee ( uid)VALUES ( '$uid')") or die(mysql_error());
     $_SESSION['uid']= $uid;
		...
		<div class="container question">
			<div class="col-xs-12 col-sm-8 col-md-8 col-xs-offset-4 col-sm-offset-3 col-md-offset-3">
				...
				<form class="form-horizontal" role="form" id='login' method="post" action="result.php">
					<?php
					$number_question = 1;
					$row = mysqli_query( $con, "select * from quiz_event ORDER BY RAND()");
...
if(isset($_POST[1])){ 
  ...
  // echo $query;exit;
   ...
   $response=mysql_query("select id,ans from quiz_event where id IN($order) ORDER BY FIELD(id,$order)")   or die(mysql_error());
   $right_op=0;
   $wrong_op=0;
   $unoped=0;
